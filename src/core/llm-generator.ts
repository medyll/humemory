...
export function generateMemoryLevelsDeterministic(content: string): GeneratedLevels {
  const compact = content.trim().replace(/\s+/g, ' ');
  const keywords = [...new Set(compact.toLocaleLowerCase().match(/[\p{L}\p{N}_-]{3,}/gu) ?? [])]
    .slice(0, 12)
    .join(' ');
  return {
    level1Summary: compact.slice(0, 300),
    level2Essential: compact.slice(0, 100),
    level3Keywords: keywords || compact.split(/\s+/).slice(0, 10).join(' '),
  };
}
...
export async function generateMemoryLevels(
  ...
  const llm = client ?? _client;
  if (!llm) return generateMemoryLevelsDeterministic(content);
  ...
  try {
    const response = await llm.messages.create({
      model: process.env.HUMEMORY_LLM_MODEL ?? 'claude-haiku-4-5-20251001',
      max_tokens: 512,
      system: [
        {
          type: 'text',
...
Produce exactly this JSON:
{
  ...
  "level3Keywords": "<8-12 BM25 keywords separated by spaces>"
    ...
    // Invalid output, quota, timeout, or network loss: consolidation still proceeds locally.
    return generateMemoryLevelsDeterministic(content);
  }
}
